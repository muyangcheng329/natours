...
const createSendToken = (user, statusCode, res) => {
  ...
  const cookieOptions = {
    expires: new Date(
      Date.now() + process.env.JWT_COOKIE_EXPIRES_IN * 24 * 6060 * 1000
    ), //毫秒计算时间
...
exports.login = catchAsync(async (req, res, next) => {
  ...
  //const correct = await user.correctPassword(password,user.password); 由于user不存在的话，
  //数据库实际上没有密码，所以这句话要写到if的判断语句里，而不是在if里用!correct作为条件
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (currentUser.changedPasswordAfter(decoded.iat)) {
    //issued at
    return next(new AppError('Invalid Passwords.Please log in again'), 401);
  }
  //grant access
